...
contract ReputationLedger is ZamaEthereumConfig {
    ...
    function _ensureRecord(address freelancer) internal {
        if (!_records[freelancer].exists) {
            ...
            _records[freelancer] = FreelancerRecord({
                totalEarnings: zeroEarnings, ratingSum: zeroRating, ratingCount: 0, completedJobs: 0, exists: true
            });
        }
    }

    function submitRating(address freelancer, externalEuint8 encryptedRating, bytes calldata inputProof) external {
    ...
    function recordEarnings(address freelancer, externalEuint64 encryptedEarning, bytes calldata inputProof) external {
        require(msg.sender == jobRegistry, "ReputationLedger: only JobRegistry");
        _ensureRecord(freelancer);
    ...
    function proveRatingAbove(address freelancer, uint32 threshold) external returns (ebool result) {
    ...
    function proveEarningsAbove(address freelancer, uint64 thresholdInWei) external returns (ebool result) {
        require(_records[freelancer].exists, "ReputationLedger: freelancer not found");
    ...
    function getEncryptedEarnings(address freelancer) external view returns (euint64) {
        require(_records[freelancer].exists, "ReputationLedger: not found");
        return _records[freelancer].totalEarnings;
    }

    function getEncryptedRatingSum(address freelancer) external view returns (euint32) {
        require(_records[freelancer].exists, "ReputationLedger: not found");
        return _records[freelancer].ratingSum;
    }

    function getPublicStats(address freelancer) external view returns (uint32 ratingCount, uint256 completedJobs) {
        FreelancerRecord storage rec = _records[freelancer];
